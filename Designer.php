...
<head>
    ...
    <meta name="designer" content="Amey Thakur">
    <title>Digital Fasionstore</title>
    <!-- Bootstrap -->
    ...
    <style>
        ...
        #articles .row{margin-top:30px;margin-bottom:30px;font-weight:800;}
        @media only screen and (max-width: 760px) { #articles .row{margin-top:10px;}}
   </style>
</head>
<body>
        ...
        <div id="searchbox" class="container-fluid" style="width:112%;margin-left:-6%;margin-right:-6%;">
            <div>
                <form role="search" action="Result.php" method="post">
                    <input type="text" class="form-control" name="keyword" placeholder="Search for a Book , Designer Or Category" style="width:80%;margin:20px 10% 20px 10%;">
                </form>
            </div>
        </div>
    ...
    include "dbconnect.php";
        if(isset($_GET['value']))
        {
           $_SESSION['designer']=$_GET['value'];
        }
    $designer=$_SESSION['designer'];
    if(isset($_POST['sort']))
    {
        if($_POST['sort']=="price")
                {   $query = "SELECT * FROM products WHERE Designer='$designer' ORDER BY Price";
                    $result = mysqli_query ($con,$query)or die(mysqli_error($con));
                }
        else
        if($_POST['sort']=="priceh")
                {   $query = "SELECT * FROM products WHERE Designer='$designer' ORDER BY Price DESC";
                    $result = mysqli_query ($con,$query)or die(mysqli_error($con));
                }
        else
        if($_POST['sort']=="discount")
                {   $query = "SELECT * FROM products WHERE Designer='$designer' ORDER BY Discount DESC";
                    $result = mysqli_query ($con,$query)or die(mysqli_error($con));
                }
        else
        if($_POST['sort']=="discountl")
                {   $query = "SELECT * FROM products WHERE Designer='$designer' ORDER BY Discount";
                    $result = mysqli_query ($con,$query)or die(mysqli_error($con));
                }
    }
    else
                  $query = "SELECT * FROM products WHERE Designer='$designer'";
                  $result = mysqli_query ($con,$query)or die(mysql_error());
    $i=0;
    echo '<div class="container-fluid" id="articles">
        <div class="row">
          <div class="col-xs-12 text-center" id="heading">
                 <h2 style="color:#D67B22;text-transform:uppercase;margin-bottom:0px;"> '. $designer .' STORE </h2>
           </div>
        </div>
        <div class="container fluid">
             <div class="row">
